let program =
	'3,1033,1008,1033,1,1032,1005,1032,31,1008,1033,2,1032,1005,1032,58,1008,1033,3,1032,1005,1032,81,1008,1033,4,1032,1005,1032,104,99,101,0,1034,1039,1001,1036,0,1041,1001,1035,-1,1040,1008,1038,0,1043,102,-1,1043,1032,1,1037,1032,1042,1105,1,124,101,0,1034,1039,101,0,1036,1041,1001,1035,1,1040,1008,1038,0,1043,1,1037,1038,1042,1106,0,124,1001,1034,-1,1039,1008,1036,0,1041,1001,1035,0,1040,1001,1038,0,1043,1002,1037,1,1042,1105,1,124,1001,1034,1,1039,1008,1036,0,1041,102,1,1035,1040,101,0,1038,1043,1002,1037,1,1042,1006,1039,217,1006,1040,217,1008,1039,40,1032,1005,1032,217,1008,1040,40,1032,1005,1032,217,1008,1039,1,1032,1006,1032,165,1008,1040,33,1032,1006,1032,165,1101,0,2,1044,1106,0,224,2,1041,1043,1032,1006,1032,179,1101,1,0,1044,1106,0,224,1,1041,1043,1032,1006,1032,217,1,1042,1043,1032,1001,1032,-1,1032,1002,1032,39,1032,1,1032,1039,1032,101,-1,1032,1032,101,252,1032,211,1007,0,43,1044,1105,1,224,1101,0,0,1044,1106,0,224,1006,1044,247,1002,1039,1,1034,1002,1040,1,1035,102,1,1041,1036,1001,1043,0,1038,101,0,1042,1037,4,1044,1105,1,0,13,30,60,64,5,28,36,24,67,12,1,67,32,39,14,78,29,17,38,88,79,9,62,25,15,18,88,25,7,81,38,41,10,69,86,32,11,33,1,10,22,84,14,92,48,79,10,3,62,33,61,13,93,78,20,63,68,17,80,34,12,8,23,61,90,51,17,84,37,46,64,25,3,73,19,45,99,41,62,21,77,8,17,89,9,13,84,75,85,14,53,60,6,29,76,63,14,23,63,61,93,72,17,41,28,94,5,3,19,47,57,55,14,34,38,79,85,40,13,22,99,67,72,15,62,15,6,63,3,90,2,87,20,84,15,50,70,27,18,78,21,70,48,52,2,99,92,55,3,46,41,93,99,88,13,39,4,45,71,3,96,1,91,59,31,53,23,25,82,32,50,16,60,38,78,34,59,30,15,51,92,3,22,26,62,60,37,42,74,28,21,76,7,24,70,18,40,11,81,41,9,73,62,12,66,81,9,3,74,62,11,6,56,16,34,20,78,79,1,97,17,39,87,15,12,77,94,28,22,66,45,59,39,2,6,52,6,72,49,17,92,15,86,18,92,79,67,20,22,72,10,72,3,52,26,77,78,41,97,36,59,88,24,57,12,38,90,53,14,38,67,2,36,44,93,99,10,41,49,3,16,7,63,32,11,15,81,12,91,39,62,19,83,6,91,28,19,80,38,23,63,31,71,14,58,8,21,71,21,21,81,38,26,32,29,82,52,28,72,54,97,41,65,96,75,1,48,28,80,66,25,47,49,29,87,51,12,50,70,36,60,81,29,77,76,55,25,40,45,83,91,26,72,99,12,47,11,20,27,52,9,98,17,99,27,37,62,25,3,15,73,66,22,5,85,5,20,98,20,38,62,78,21,16,59,28,98,38,31,2,40,46,87,14,48,33,80,48,36,27,56,21,1,50,83,3,61,92,20,52,16,50,10,86,9,98,39,56,25,50,42,39,91,81,56,25,70,44,24,15,99,4,20,55,12,98,27,65,20,77,97,76,36,42,87,6,11,79,65,16,65,44,13,90,13,48,79,13,95,60,19,55,24,66,4,53,11,23,68,14,97,53,45,14,16,93,18,29,83,5,6,77,19,70,97,34,20,70,52,11,74,14,72,10,36,44,33,45,19,38,36,77,5,37,51,1,55,17,2,48,23,18,2,34,90,97,24,30,51,66,33,70,51,37,31,51,37,65,55,18,8,66,4,65,62,26,93,29,88,3,75,73,24,23,67,1,13,68,7,36,87,62,48,1,31,45,28,62,86,24,98,1,59,49,37,26,62,36,44,66,18,17,97,92,40,36,65,80,84,5,84,6,79,87,36,31,96,15,71,96,2,72,11,81,95,94,41,54,31,58,25,74,24,51,81,38,32,73,22,96,40,62,22,59,74,39,25,86,2,55,20,61,40,37,88,69,1,60,42,18,31,54,13,27,19,93,34,41,99,33,89,20,16,52,84,32,94,31,6,61,25,1,61,1,38,78,87,39,31,39,26,68,42,36,2,94,66,2,67,30,80,2,95,65,40,54,50,33,11,23,97,89,1,31,56,9,35,49,92,55,23,84,48,91,20,7,72,25,55,3,85,3,16,40,90,22,99,44,38,86,98,11,76,26,76,13,82,80,24,93,4,15,64,95,58,15,85,25,57,29,66,3,66,19,98,57,24,44,59,35,76,48,31,92,33,94,68,56,41,45,15,46,5,68,15,65,34,73,49,68,17,78,28,80,24,59,26,74,21,52,1,94,5,61,41,88,37,56,1,49,0,0,21,21,1,10,1,0,0,0,0,0,0';

//const Computer = require('../IntComputer.js');

let dataArray = program
	.trim()
	.split(',')
	.map((x) => {
		return +x;
	});

let x = 0;
let y = 0;
let direction = 'E';
let path = new Map();

let inputs = new Map();
inputs.set('N', 1);
inputs.set('S', 2);
inputs.set('W', 3);
inputs.set('E', 4);

let floorPlan = new Array();
floorPlan[x] = new Array();
floorPlan[x][y] = 1;
path.set(x + ',' + y, 1);

let output = -1;
let robot = new Computer(dataArray, true);
let counter = 0;

let endPoint = 1000;

while (output !== 2 && counter < endPoint) {
	let roboInput = inputs.get(direction);

	robot.receiveInput(roboInput);

	while (robot.running && counter < endPoint && output !== 2) {
		if (robot.paused) {
			counter++;
			output = +robot.returnOutputArray()[0];

			switch (direction) {
				case 'N':
					floorPlan[x][y - 1] = output;
					break;
				case 'E':
					if (floorPlan[x + 1]) {
						floorPlan[x + 1][y] = output;
					} else {
						floorPlan[x + 1] = new Array();
						floorPlan[x + 1][y] = output;
					}
					break;
				case 'S':
					floorPlan[x][y + 1] = output;
					break;
				case 'W':
					if (floorPlan[x - 1]) {
						floorPlan[x - 1][y] = output;
					} else {
						floorPlan[x - 1] = new Array();
						floorPlan[x - 1][y] = output;
					}
					break;
				default:
					break;
			}

			if (output === 2) {
				break;
			} else if (output === 1) {
				switch (direction) {
					case 'N':
						y--;
						break;
					case 'E':
						x++;
						break;
					case 'S':
						y++;
						break;
					default:
						x--;
						break;
				}

				let foundPath = +path.get(x + ',' + y);

				if (foundPath) {
					path.set(x + ',' + y, ++foundPath);
					calculateDirection();
				} else {
					path.set(x + ',' + y, 1);
				}
			} else {
				calculateDirection();
			}
			robot.receiveInput(inputs.get(direction));
		}
	}
}

if (output === 2) {
	let stepCount = retracePath();
	console.log('STEP1: ' + stepCount);
}

let xCoords = Object.keys(floorPlan);
xCoords.sort((a, b) => {
	return +a < +b ? -1 : +a === +b ? 0 : 1;
});
let minX = +xCoords[0];
let maxX = +xCoords[xCoords.length - 1];
let minY = Infinity;
let maxY = -Infinity;

// DRAWMAP

xCoords.forEach((element) => {
	let idx = +element;

	let yCoords = Object.keys(floorPlan[idx]);
	yCoords.sort((a, b) => {
		return +a < +b ? -1 : +a === +b ? 0 : 1;
	});

	minY = +yCoords[0] < minY ? +yCoords[0] : minY;
	maxY = +yCoords[yCoords.length - 1] > maxY ? +yCoords[yCoords.length - 1] : maxY;
});

for (let idy = minY; idy < maxY + 1; idy++) {
	let outString = ('' + idy).padStart(3, ' ') + ': ';
	for (let idx = minX; idx < maxX + 1; idx++) {
		if (idx === x && idy === y) {
			if (floorPlan[idx][idy] === 2) {
				outString += 'B';
			} else {
				outString += 'X';
			}
		} else if (floorPlan[idx][idy] === 1) {
			outString += String.fromCharCode(9675);
		} else if (floorPlan[idx][idy] === 0) {
			outString += String.fromCharCode(9679);
		} else {
			// output += (String.fromCharCode(9708));
			outString += '.';
		}
	}
	console.log(outString);
}
// console.log(floorPlan);
// console.log([x, y, output, counter]);
// console.log([minX, maxX, minY, maxY]);
// console.log(path);

fillOxygen();

// FUNCTIONS

function calculateDirection() {
	let neighbours = calculateNeighbours();
	let foundPath = +path.get(x + ',' + y);

	if (neighbours[direction] === 0) {
		switch (direction) {
			case 'N':
				direction = 'E';
				break;
			case 'E':
				direction = 'S';
				break;
			case 'S':
				direction = 'W';
				break;
			default:
				direction = 'N';
				break;
		}

		if (neighbours[direction] === 0 || neighbours[direction] === 1) {
			let posDirections = new Array();
			let beenDirections = new Array();
			Object.keys(neighbours).forEach((element) => {
				if (neighbours[element] === -1) {
					posDirections.push(element);
				} else if (neighbours[element] === 1) {
					beenDirections.push(element);
				}
			});

			if (posDirections.length > 0) {
				direction = posDirections.pop();
			} else {
				let previousBeen = Infinity;
				beenDirections.forEach((element) => {
					let newX = x;
					let newY = y;

					switch (element) {
						case 'N':
							newY--;
							break;
						case 'E':
							newX++;
							break;
						case 'S':
							newY++;
							break;
						default:
							newX--;
							break;
					}

					if (path.get(newX + ',' + newY) < previousBeen) {
						previousBeen = path.get(newX + ',' + newY);
						direction = element;
					}
				});
			}
		}
		return;
	} else if (neighbours[direction] === 1 && foundPath) {
		let posDirections = new Array();
		let beenDirections = new Array();
		Object.keys(neighbours).forEach((element) => {
			if (neighbours[element] === -1) {
				posDirections.push(element);
			} else if (neighbours[element] === 1) {
				beenDirections.push(element);
			}
		});

		if (posDirections.length > 0) {
			direction = posDirections.pop();
		} else {
			let previousBeen = Infinity;
			beenDirections.forEach((element) => {
				let newX = x;
				let newY = y;

				switch (element) {
					case 'N':
						newY--;
						break;
					case 'E':
						newX++;
						break;
					case 'S':
						newY++;
						break;
					default:
						newX--;
						break;
				}

				if (path.get(newX + ',' + newY) < previousBeen) {
					previousBeen = path.get(newX + ',' + newY);
					direction = element;
				}
			});
		}
	} else {
		console.log(['OOPS', counter]);
	}
	return;
}

function calculateNeighbours() {
	let neighbours = {};

	if (floorPlan[x]) {
		neighbours['N'] = floorPlan[x][y - 1] !== undefined ? floorPlan[x][y - 1] : -1;
		neighbours['S'] = floorPlan[x][y + 1] !== undefined ? floorPlan[x][y + 1] : -1;
	} else {
		neighbours['N'] = -1;
		neighbours['S'] = -1;
	}

	if (floorPlan[x + 1]) {
		neighbours['E'] = floorPlan[x + 1][y] !== undefined ? floorPlan[x + 1][y] : -1;
	} else {
		neighbours['E'] = -1;
	}

	if (floorPlan[x - 1]) {
		neighbours['W'] = floorPlan[x - 1][y] !== undefined ? floorPlan[x - 1][y] : -1;
	} else {
		neighbours['W'] = -1;
	}

	return neighbours;
}

function retracePath() {
	let steps = 1;
	let stepX = x;
	let stepY = y;
	let retracePath = {};
	retracePath[stepX + ',' + stepY] = 1;

	while ((stepX !== 0 || stepY !== 0) && steps < counter) {
		steps++;
		let neighbours = new Array();

		neighbours.push([path.get(stepX - 1 + ',' + stepY), stepX - 1, stepY]);
		neighbours.push([path.get(stepX + 1 + ',' + stepY), stepX + 1, stepY]);
		neighbours.push([path.get(stepX + ',' + (stepY - 1)), stepX, stepY - 1]);
		neighbours.push([path.get(stepX + ',' + (stepY + 1)), stepX, stepY + 1]);

		let minStep = Infinity;

		neighbours.forEach((element) => {
			if (minStep > element[0] && !retracePath[element[1] + ',' + element[2]]) {
				minStep = element[0];
				stepX = element[1];
				stepY = element[2];
			}
		});
		retracePath[stepX + ',' + stepY] = 1;
	}

	return steps;
}

function fillOxygen() {
	let oxygenCounter = 1;
	let oxygenRooms = new Array();

	let filledRooms = new Array();
	filledRooms[x] = new Array();
	filledRooms[x][y] = 1;

	oxygenRooms[x] = new Array();
	oxygenRooms[x][y] = 1;

	let newEndPoints = calculateEndPoints(oxygenRooms);

	while (newEndPoints.length > 0 && oxygenCounter < 6) {
		oxygenCounter++;

		oxygenRooms = new Array();

		newEndPoints.forEach((endPoint) => {
			if (oxygenRooms[endPoint.x]) {
				oxygenRooms[endPoint.x][endPoint.y] = 1;
			} else {
				oxygenRooms[endPoint.x] = new Array();
				oxygenRooms[endPoint.x][endPoint.y] = 1;
			}
		});

		newEndPoints = calculateEndPoints(oxygenRooms);
		console.log(newEndPoints);
	}
}

function calculateEndPoints(filledRooms) {
	let endPoints = new Array();

	let xPoints = Object.keys(filledRooms).map((x) => {
		return +x;
	});

	xPoints.forEach((xCoord) => {
		let yPoints = Object.keys(filledRooms[xCoord]).map((x) => {
			return +x;
		});

		yPoints.forEach((yCoord) => {
			if (floorPlan[xCoord]) {
				if (floorPlan[xCoord][yCoord - 1] !== undefined && floorPlan[xCoord][yCoord - 1] === 1) {
					endPoints.push({ x: xCoord, y: yCoord - 1 });
				}

				if (floorPlan[xCoord][yCoord + 1] !== undefined && floorPlan[xCoord][yCoord + 1] === 1) {
					endPoints.push({ x: xCoord, y: yCoord + 1 });
				}
			}

			if (floorPlan[xCoord - 1]) {
				if (floorPlan[xCoord - 1][yCoord] !== undefined && floorPlan[xCoord - 1][yCoord] === 1) {
					endPoints.push({ x: xCoord - 1, y: yCoord });
				}
			}

			if (floorPlan[xCoord + 1]) {
				if (floorPlan[xCoord + 1][yCoord] !== undefined && floorPlan[xCoord + 1][yCoord] === 1) {
					endPoints.push({ x: xCoord + 1, y: yCoord });
				}
			}
		});
	});

	return endPoints;
}
