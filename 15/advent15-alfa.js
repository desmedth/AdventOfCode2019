...
xCoords.sort((a,b) => { return +a<+b?-1:(+a===+b?0:1);});
let minX = +xCoords[0];
...
xCoords.forEach(element => {
  ...
  yCoords.sort((a,b) => { return +a<+b?-1:(+a===+b?0:1);});
...
{
  let output = (''+idx).padStart(3,' ')+': ';
  for (let idy = minY; idy < maxY+1; idy++)
  {
    if (floorPlan[idx][idy] === 1)
    {
      output += (String.fromCharCode(9675));
    }
    else if (floorPlan[idx][idy] === 0)
    {
      output += (String.fromCharCode(9679));
    }
    else{
...
function calculateDirection()
{
  ...
  if (neighbours[direction] === -1 || neighbours[direction] === 1)
  {
    return;
  }
  else if (neighbours[direction] === 0)
  {
    switch (direction) {
      case 'N':
        direction = 'E';
